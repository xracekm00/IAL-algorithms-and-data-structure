...
void Stack_Init(Stack *stack) {
	//Checks if param is not NULL and allocation succeded
	if (stack == NULL || (stack->array = malloc(sizeof(char) * STACK_SIZE)) == NULL){
...
bool Stack_IsEmpty(const Stack *stack) {
	//ternary operator
	return (stack->topIndex >= 0)?false:true;
}
...
void Stack_Pop(Stack *stack) {
	//To make sure the top index isn't less than -1
	if (!Stack_IsEmpty(stack)){
		stack->topIndex--;
	}
}
